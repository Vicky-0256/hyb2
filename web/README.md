...
The structure workspace loads a pinned ViennaRNA 2.7.2 RNAlib build in a dedicated Web Worker. It reports a global MFE in kcal/mol, dot-bracket notation, CT and base-pair exports, and interactive SVG views for arc, radial, circular-contact, and symmetric base-pair matrix layouts. Arc remains the default; all views share nucleotide inspection, HYB/RNAcofold evidence colouring, CPLfold layer styling, and SVG/PNG export. For ViennaRNA, the normal UI accepts up to 2,000 nt; users may explicitly continue up to 3,000 nt after a browser-memory warning. CPLfold is the default engine and starts in pseudoknot-free mode; its browser path has a separate 500-nt hard safety ceiling and a session-specific capacity recommendation. Users may enable CPLfold pseudoknot search explicitly. ViennaRNA remains available for nested MFE, HYB-guided RNAcofold evidence, and manual hard base pairs; an expert may instead choose **Manual hard base pairs** and enter one 1-based `i-j` pair per line, relative to the prepared sequence.
