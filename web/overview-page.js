(function () {
  ...
  function renderMethods() {
    return [
      ...
      '        <p class="notice">The structure workspace provides reference-region input for HYB-guided modes and supports HYB record or pasted sequence input in compatible sequence-only and ViennaRNA modes. Plain and manual modes run pinned ViennaRNA MFE WebAssembly. HYB-guided ViennaRNA reproduces the RNAcofold evidence, compatible F-constraint, randomised-fold and COMRADES-scoring chain. A separate pure-Python CPLfold/Pyodide engine predicts and visualises nested and pseudoknotted candidates from a sequence or a bonus matrix built from all eligible HYB rows in one selected reference region; the browser measures a conservative session-specific capacity above a 75 nt baseline and enforces a 500 nt hard safety ceiling. UNAFold remains an optional external CLI compatibility path.</p>',
      "      </section>"
    ].join("");
  }
